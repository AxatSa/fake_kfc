...
def start_message(message):
    ...
    reply_markup=buttons.langeage_button())
    bot.register_next_step_handler(message, choise_language)

def choise_language(message):
    if message.text.lower() == 'eng':
        bot.send_message(message.from_user.id, 'Lets speak english',
        reply_markup=telebot.types.ReplyKeyboardRemove())
        bot.register_next_step_handler(message, eng_func)
    else:
        bot.send_message(message.from_user.id, 'Продолжим на русском ',
        reply_markup=telebot.types.ReplyKeyboardRemove())
        bot.register_next_step_handler(message,rus_func)
def eng_func(message):
    print(message.from_user.id)
def rus_func(message):
    print(message.from_user.first_name)

    # получаем тг ид и проврям
